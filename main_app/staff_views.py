                ...
                if existing_attendance:
                    ...
                    if existing_report:
                        ...
                        if is_update:
                            ...
                            existing_report.status = attendance_status
                            existing_report.save()
                            
                            return JsonResponse({
                                'status': 'success',
                                'message': f'Attendance status updated successfully ({"Present" if attendance_status else "Absent"})',
                                'student_name': f"{student.admin.first_name} {student.admin.last_name}",
                                'was_updated': True,
                                'old_status': 'Present' if old_status else 'Absent',
                                'new_status': 'Present' if attendance_status else 'Absent'
                            })
                        else:
                            # Inform the user that attendance is already marked and ask if they want to update
                            return JsonResponse({
                                'status': 'duplicate',
                                'message': 'Attendance already marked for this student today',
                                'student_name': f"{student.admin.first_name} {student.admin.last_name}",
                                'student_id': student.id,
                                'current_status': 'Present' if existing_report.status else 'Absent',
    ...
    try:
        ...
        for attendance in attendance_data:
            data = {
                ...
                "student_code": attendance.student.admin.student_code,  # Add the student code
...
def delete_attendance(request):
    ...
    try:
        # Get attendance date ID from request
        attendance_date_id = request.POST.get('attendance_date_id')
        
        if not attendance_date_id:
            return JsonResponse({
                'status': 'error',
                'message': 'Missing attendance_date_id parameter'
            }, status=400)
        
        # Get the attendance record
        attendance = get_object_or_404(Attendance, id=attendance_date_id)
        
        # Check if this attendance record belongs to a subject taught by this staff
        staff = Staff.objects.get(admin=request.user)
        if attendance.subject.staff != staff:
            return JsonResponse({
                'status': 'error', 
                'message': 'You are not authorized to delete this attendance record'
            }, status=403)
        
        # First delete all attendance reports linked to this attendance
        reports_count = AttendanceReport.objects.filter(attendance=attendance).count()
        ...
        return JsonResponse({
            'status': 'success',
            'message': f'Attendance record deleted successfully with {reports_count} student records',
        })
        ...
        for attendance in attendance_records:
            ...
            for report in reports:
                ...
                student_details.append({
                    'id': student.id,
                    'name': f"{student.admin.first_name} {student.admin.last_name}",
                    'code': student.admin.student_code,  # Add student code
